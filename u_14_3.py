...
button_il_dyn = {}
for i in range(4):
    button_il_dyn[i] = aiogram.types.InlineKeyboardButton(text=f'Продукт {i+1}', callback_data="product_buying")
...
for i in range(1,5):
    cursor.execute("INSERT INTO Products (name, about, cost, stock) VALUES (?, ?, ?, ?)", (f"Product{i}",
                                                                                             f"about_example{i}",
                                                                                             f"{i*10}", f"2"))
...
async def get_buying_list(message):
    jpg_files = get_file_paths_pattern('./pict', '*.jpg')
    for i in range(len(jpg_files)):
        cursor.execute("SELECT name, about, cost FROM Products WHERE name == ?", (f"Product{i+1}",))
        prods = cursor.fetchall()
        for prod in prods:
            text_product = f"Название: {prod[0]} | Описание: {prod[1]} | Цена: {prod[2]}"
        await message.answer(text_product)
        with open(jpg_files[i], 'rb') as img:
